...
class use_symmetric_memory:
    ...
    def __enter__(self):
        if self.is_graph_capture:
            print(f"use_symmetric_memory: endAllocateCurrentStreamToPool ={torch.cuda.MemPoolContext.active_pool()}")
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        for segment in get_nccl_mem_pool().snapshot():
            if segment['address'] not in _registered_base_addrs:
                self.pynccl.register_comm_window_raw(segment['address'], segment['total_size'])
...
def test(size, pynccl_comm):
    ...
    pynccl_comm.all_reduce(symm_input)
